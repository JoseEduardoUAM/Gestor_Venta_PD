  ...
  public static ArrayList<Inventory> getAll(){
    ...
    try {
        ...
        while( result.next() ){
          ...
          State state;
          if( result.getInt(2) == 2 ) state = Devolution.get();
          else if( result.getInt(2) == 3 ) state = Reparacion.get();
          else if( result.getInt(2) == 4 ) state = Sell.get();
          else state = Stored.get();

          Product product;
          if( result.getInt(3) != 0 ) product = SmartphoneProxy.get().getProduct( result.getInt(3) );
          else if( result.getInt(4) != 0 ) product = SmartwatchProxy.get().getProduct( result.getInt(4) );
          else if( result.getInt(5) != 0 ) product = TabletProxy.get().getProduct( result.getInt(5) );
          else if( result.getInt(6) != 0 ) product = SmartTVProxy.get().getProduct( result.getInt(6) );
          else product = null;

          if( product != null )
            inventories.add(
              new Inventory( result.getInt(1) , state, product, result.getFloat(7), result.getString(8).split("\\[|\\]|,") )
            );
